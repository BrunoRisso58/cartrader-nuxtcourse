    ...
    <div class="shadow border w-64 mr-10 z-30 h-[190px]">
        ...
        <div class="p-5 flex justify-between relative cursor-pointer border-b">
            ...
            <div v-if="modal.make" class="absolute border shadow left-56 p-5 top-1 -m-1 w-[600px] flex justify-between flex-wrap bg-white">
                <h4 @click="onChangeMake(make)" class="w-1/3" v-for="make in makes" :key="make">{{ make }}</h4>
            </div>
        </div>
        <!-- MAKE END -->
        ...
        <div class="p-5 flex justify-between relative cursor-pointer border-b">
            ...
            <div v-if="modal.price" class="absolute border shadow left-56 p-5 top-1 -m-1 bg-white">
                <input class="border p-1 rounded mb-2" type="number" placeholder="Min" v-model="priceRange.min">
                <input class="border p-1 rounded mb-2" type="number" placeholder="Max" v-model="priceRange.max">
